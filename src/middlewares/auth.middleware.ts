...
//const response = await axios.get(`http://localhost:3000/can-do/${permissions}_${permissionController}`, {
...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    try {
      ...
      const response = await axios.get(`http://localhost:3001/users/can-do/${permissions} ${permissionController}`, {
        headers: {
